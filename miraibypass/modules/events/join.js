...
module.exports.run = async function({ api, event,Threads }) {
	...
	if  (checkban.includes(checkban[0])) return
		...
		api.changeNickname(`[ ${global.config.PREFIX} ] • ${(!global.config.BOTNAME) ? "ThienNhanCute" : global.config.BOTNAME}`, threadID, api.getCurrentUserID());
		return api.sendMessage(`Kết nối thành công, Vui lòng sử dụng !help để xem tất cả lệnh:3`, threadID);
	}
	else {
		try {
